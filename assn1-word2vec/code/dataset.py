...
def build_vocab(
    ...
    for token in word_freq_dict:
        if word_freq_dict[token]>=min_freq:
            idx = len(word2idx)
            word2idx[token] = idx
            word_freq.append(word_freq_dict[token])
            idx2word.append(token)
        else:
            word_freq[UNK_idx] = word_freq[UNK_idx] + word_freq_dict[token]
...
def skipgram(
    ...
    outside_words: List[str] = sentence[max(0,center_word_loc-window_size):center_word_loc] + \
    sentence[center_word_loc+1:center_word_loc+window_size+1]
